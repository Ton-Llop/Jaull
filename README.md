...
A composite score over six normalised components, weighted and then renormalised to 1.0 (`recommendation/policies.py`):
...
The chosen priority shifts these — *quality* raises task match, *speed* and *memory* raise hardware fit — and the whole total is then scaled by the estimate's confidence, so a model we could barely measure cannot outrank one we measured properly.

Compatibility is a hard gate, not just a weight: a model assessed `insufficient` never appears at all, and one assessed `unknown` can only ever be a flagged low-confidence alternative. Ties break deterministically (status → confidence → downloads → `repo_id`), so the same inputs always give the same report.

Every reason and warning is generated by rules in `recommendation/explanations.py`. No language model is involved in ranking or in writing the explanations.
...
```bash
uv run local-ai-check                             # opens the TUI when run in a terminal
...
Running `local-ai-check` with no subcommand opens the TUI **only when stdin and stdout are both a terminal**. Piped, redirected or CI invocations keep printing help exactly as before, so existing scripts are unaffected.
