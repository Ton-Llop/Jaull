...
from local_ai_check.recommendation import policies, report
...
from local_ai_check.tui.widgets.score_bar import ScoreBar
...
class RecommendationResultsScreen(Screen[None]):
    ...
    def _compose_results(self) -> ComposeResult:
        ...
        if config is not None:
            yield CliEquivalent(_equivalent_cli(primary.repo_id, config))
    ...
    def _prompt_export(self) -> None:
        ...
        container.mount(
            Input(value="local-ai-check-report.json", id="res-export-path")
        )
...
def _equivalent_cli(repo_id: str, config: object) -> str:
    from local_ai_check.domain.inference import InferenceConfiguration

    assert isinstance(config, InferenceConfiguration)
    parts = ["local-ai-check estimate", repo_id]
