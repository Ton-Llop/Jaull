...
from local_ai_check.recommendation.models import ScoreBreakdown
...
def score_breakdown(
    evaluated: EvaluatedCandidate, priority: RecommendationPriority
) -> ScoreBreakdown:
    weights = weights_for(priority)
    parts = {
        "hardware_fit": evaluated.hardware_fit_score,
...
def sort_candidates(
    evaluated: list[EvaluatedCandidate], priority: RecommendationPriority
    ...
    """
    scored = [(item, score_breakdown(item, priority)) for item in evaluated]
...
def select_ranked(
    ...
    ordered = [
        pair
        for pair in sort_candidates(evaluated, priority)
